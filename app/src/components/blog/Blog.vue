...
export default {
  ...
  methods: {
    fetchBlog: function () {
      ...
      axios.get('https://api.lmm.im/v1/blogs/' + this.blogID).then(res => {
        const blog = res.data
    ...
    fetchCategory: function () {
      axios.get('https://api.lmm.im/v1/blogs/' + this.blogID + '/category').then(res => {
    ...
    fetchTags: function () {
      axios.get('https://api.lmm.im/v1/blogs/' + this.blogID + '/tags').then(res => {
        this.tags = res.data
      }).catch(e => {
        console.log(e.response.data)
      })
